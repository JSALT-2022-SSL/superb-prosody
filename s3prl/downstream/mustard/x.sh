...
lr="1e-3"
echo "Starting training dependent setup $upstream layers -3,-2,-1 with $lr"
for i in {0..4}
do
  python run_downstream.py \
      -m train -u $upstream -d mustard \
      -p result/$EXP_NAME-$lr-x/fold-$i \
...
python downstream/mustard/gather_score.py -p result/$EXP_NAME-$lr-x